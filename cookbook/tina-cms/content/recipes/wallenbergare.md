---
title: Klassiska Wallenbergare
basePortions: 4
estimatedTime: 45
rating: 5
description: >-
  En svensk lyxklassiker. Wallenbergare är en luftig färsbiff gjord på finmalen
  kalvfärs, vispgrädde och äggulor, vänd i ströbröd och stekt till gyllene
  perfektion.
heroImage: ''
ingredients:
  - name: Kalvfärs (finmalen)
    amount: 500
    unit: g
  - name: Äggulor
    amount: 4
    unit: st
  - name: Vispgrädde (kall)
    amount: 3
    unit: dl
  - name: Salt
    amount: 1.5
    unit: tsk
  - name: Vitpeppar (nimalen)
    amount: 1
    unit: krm
  - name: Ströbröd (gärna panko eller ljust bröd)
    amount: 2
    unit: dl
  - name: Smör (till stekning)
    amount: 50
    unit: g
  - name: Potatismos (till servering)
    amount: 1
    unit: st
  - name: Gröna ärter (till servering)
    amount: 200
    unit: g
  - name: Rårörda lingon (till servering)
    amount: 1
    unit: dl
  - name: Skirat smör (till servering)
    amount: 50
    unit: g
steps:
  - title: Förbered färsen
    description: >-
      Se till att alla ingredienser är riktigt kalla. Blanda kalvfärsen med salt
      och vitpeppar i en bunke. Arbeta i äggulorna, en i taget.
  - title: Tillsätt grädden
    description: >-
      Rör ner vispgrädden lite i taget. Arbeta färsen smidig men rör inte för
      mycket så att den blir varm eller tappar sin luftighet.
  - title: Forma biffarna
    description: >-
      Forma färsen till åtta stora, runda biffar. Vänd dem försiktigt i ströbröd
      på en tallrik.
  - title: Stek wallenbergarna
    description: >-
      Hetta upp smör i en stekpanna på medelvärme. Stek biffarna cirka 3–4 minuter
      per sida tills de är gyllenbruna och genomstekta men fortfarande saftiga.
  - title: Servera
    description: >-
      Servera genast med nylagat potatismos, kokta gröna ärter, rårörda lingon och
      en generös skvätt skirat smör över biffarna.
dietaryInfo:
  vegetarian: false
  vegan: false
  glutenFree: false
  dairyFree: false
  nutFree: true
  lowCarb: false
---
