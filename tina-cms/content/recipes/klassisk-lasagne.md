---
title: Klassisk lasagne
basePortions: 6
estimatedTime: 90
rating: 0
description: |
  En autentisk italiensk lasagne med hemlagad bolognese, krämig béchamel och lasagneplattor. Lagad med omsorg för balanserade smicker och perfekt textur — från Emilia-Romagna, rakt in i ditt kök.
ingredientGroups:
  - groupName: Bolognese
    ingredients:
      - name: Nötfärs
        amount: 500
        unit: g
      - name: Gul lök
        amount: 1
        unit: st
      - name: Morot
        amount: 1
        unit: st
      - name: Stjälkselleri
        amount: 1
        unit: st
      - name: Vitlöksklyftor
        amount: 2
        unit: st
      - name: Rött vin
        amount: 2
        unit: dl
      - name: Krossade tomater
        amount: 400
        unit: g
      - name: Tomatpuré
        amount: 2
        unit: msk
      - name: Mjölk
        amount: 1
        unit: dl
      - name: Olivolja
        amount: 2
        unit: msk
      - name: Salt
        unit: efter smak
      - name: Svartpeppar
        unit: efter smak
      - name: Torkad oregano
        amount: 1
        unit: tsk
      - name: Torkad basilika
        amount: 1
        unit: tsk
  - groupName: Béchamel
    ingredients:
      - name: Smör
        amount: 50
        unit: g
      - name: Vetemjöl
        amount: 50
        unit: g
      - name: Mjölk
        amount: 7
        unit: dl
      - name: Muskotnöt
        unit: nypa
      - name: Salt
        unit: efter smak
      - name: Vitpeppar
        unit: efter smak
  - groupName: Övrigt
    ingredients:
      - name: Lasagneplattor
        amount: 300
        unit: g
      - name: Riven parmesan
        amount: 100
        unit: g
      - name: Rivet mozzarella
        amount: 150
        unit: g
stepGroups:
  - groupName: Mise en place
    steps:
      - title: Förbered grönsakerna
        description: |
          Skala och finhacka gul lök, morot och stjälkselleri till samma storlek (soffritto). Finfördela vitlöksklyftorna. Mät upp samtliga ingredienser och ha dem redo.
  - groupName: Bolognese
    steps:
      - title: Svetta grönsakerna
        description: |
          Hetta upp olivoljan i en stor tjockbottnad gryta på medelvärme. Tillsätt lök, morot och selleri. Svetta 8–10 minuter tills grönsakerna är mjuka och glansiga. Tillsätt vitlöken den sista minuten.
      - title: Bryn nötfärsen
        description: |
          Höj värmen och lägg i nötfärsen. Bryn under omrörning tills den är finfördelad och genomstekt, ca 5–7 minuter. Salta och peppra lätt.
      - title: Deglacera med vin
        description: |
          Rör ner tomatpurén och fräs 1–2 minuter. Häll i rödvinet och skrapa loss stekskor från bottnen. Låt reducera 2–3 minuter tills vätskan nästan kokat bort.
      - title: Bygg såsen
        description: |
          Tillsätt krossade tomater, oregano och basilika. Rör om. Sänk värmen och låt puttra utan lock i minst 45 minuter under omrörning då och då.
      - title: Finish
        description: |
          Rör i mjölken i slutet. Detta balanserar tomaternas syra och ger köttsåsen en rundare, mjukare karaktär. Smaka av med salt och peppar.
  - groupName: Béchamel
    steps:
      - title: Gör redning
        description: |
          Smält smöret i en kastrull på medelvärme. När det slutat puttra, vispa ner vetemjölet och låt det fräs med under konstant omrörning i 1–2 minuter utan att ta färg (roux).
      - title: Spara mjölken gradvis
        description: |
          Häll i mjölken i en tunn stråle under ständig vispning för att undvika klumpar. Fortsätt vispa tills såsen tjocknar och börjar koka.
      - title: Smaka av
        description: |
          Dra kastrullen från värmen. Rör i muskotnöt, salt och vitpeppar. Konsistensen ska vara som en tunn gröt — den tjocknar ytterligare i ugnen.
  - groupName: Hopbakning och gräddning
    steps:
      - title: Förvärm ugnen
        description: |
          Sätt ugnen på 200°C varmluft.
      - title: Varva ingredienserna
        description: |
          I en smord ugnsform (ca 30×20 cm): Bred ut ett tunt lager bolognese i botten. Lägg ett lager lasagneplattor. Följ med bolognese, en sil på béchamel, riven parmesan och mozzarella. Upprepa skikten 3–4 gånger. Avsluta med ett generöst lager béchamel och ost ovanpå.
      - title: Grädda
        description: |
          Grädda mitt i ugnen i 30–35 minuter tills ytan är gyllenbrun och bubblar.
      - title: Vila och servera
        description: |
          Låt lasagnen vila 10 minuter före servering. Det gör den fastare och enklare att dela upp i snygga bitar.
dietaryInfo:
  vegetarian: false
  vegan: false
  glutenFree: false
  dairyFree: false
  nutFree: true
  lowCarb: false
---
